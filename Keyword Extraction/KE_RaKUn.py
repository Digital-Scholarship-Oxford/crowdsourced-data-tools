...
OUTPUT_FILENAME = "output_KE_RaKUn.xlsx"
COLUMN_PREFIX = 'keywords_RaKUn_RaKUn'
...
# %%
def extract_keywords_rakun(text):
    text = str(text)   # Convert to string to handle potential NaN values
    if pd.isna(text):
        return []     
    
    hyperparameters = {"num_keywords": NUMBER_OF_KEYWORDS,
                   "merge_threshold": 1.1,
                   "alpha": 0.3,
                   "token_prune_len": 3}
    ...
    keyphrases = keyword_detector.find_keywords(text, input_type="string")
...
df[COLUMN_PREFIX] = df['texts'].progress_apply(extract_keywords_rakun)    
